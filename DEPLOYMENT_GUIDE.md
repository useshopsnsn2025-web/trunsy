...
> **建议**：直接替换 `disable_functions` 整行，只保留真正危险的函数：
> ```bash
> sed -i 's/^disable_functions.*/disable_functions = chroot,chgrp,chown,ini_alter,ini_restore,dl,openlog,syslog,readlink,symlink,imap_open,apache_setenv/' /www/server/php/80/etc/php.ini
> sed -i 's/^disable_functions.*/disable_functions = chroot,chgrp,chown,ini_alter,ini_restore,dl,openlog,syslog,readlink,symlink,imap_open,apache_setenv/' /www/server/php/80/etc/php-cli.ini
...
**1) ValidateService 报错 `Call to undefined method think\Validate::maker()`：**
...
**2) `runtime/services.php` 不存在（多应用模式不生效，报 `class not exists: api`）：**
...
**3) Validate `filter_var()` 报错 `Argument #3 must be of type array|int, null given`（PHP 8.0 兼容性）：**

```bash
sed -i 's/return false !== filter_var($value, is_int($rule) ? $rule : filter_id($rule), $param);/return false !== filter_var($value, is_int($rule) ? $rule : filter_id($rule), $param ?? []);/' /www/wwwroot/turnsy/bbo-server/vendor/topthink/think-validate/src/Validate.php
```
...
> **注意**：如果构建报错 `ENOTDIR: scandir '.user.ini'`，是宝塔自动在 `dist` 目录创建了 `.user.ini` 文件。解决：
> ```bash
> chattr -i /www/wwwroot/turnsy/bbo-admin/dist/.user.ini
> rm -f /www/wwwroot/turnsy/bbo-admin/dist/.user.ini
> npm run build
> ```
...
# Admin 前端重新构建（先删除 .user.ini 避免构建失败）
cd ../bbo-admin
chattr -i dist/.user.ini 2>/dev/null
rm -f dist/.user.ini 2>/dev/null
npm install
npm run build
...
```bash
# 开启
sed -i 's/APP_DEBUG = false/APP_DEBUG = true/' /www/wwwroot/turnsy/bbo-server/.env

# 复现错误后查看具体信息，修复完立即关闭
sed -i 's/APP_DEBUG = true/APP_DEBUG = false/' /www/wwwroot/turnsy/bbo-server/.env
```

### 10.2 API 返回 500 `class not exists: api`

多应用服务未注册。检查 `runtime/services.php` 是否存在且内容正确（见 3.6 节）。

### 10.3 登录/注册返回 500 `filter_var() Argument #3 must be of type array|int, null given`

think-validate 与 PHP 8.0 不兼容。修复方法见 3.6 节第 3 点。

### 10.4 采集/导出卡住没有进度

后台 PHP 进程未启动，常见原因：

1. **PHP 函数被禁用**：确保 `popen`、`pclose`、`exec`、`shell_exec` 已解除禁用
2. **`PHP_BINARY` 为空**：PHP-FPM 环境下可能获取不到正确的 PHP CLI 路径，需要硬编码为 `/www/server/php/80/bin/php`
3. **`crawl_worker.php` 不存在**：确认文件已提交到 Git 仓库（可能被 `.gitignore` 排除了，需要 `git add -f`）

### 10.5 图片/静态资源 404

1. **storage 目录未上传**：需要将本地的 `bbo-server/public/storage/` 上传到服务器
2. **Nginx 静态资源规则**：确保配置了 `@backend_storage` fallback（见 4.3 节）

### 10.6 Admin 构建失败 `ENOTDIR: scandir '.user.ini'`

宝塔在 `dist` 目录创建了被锁定的 `.user.ini`。解决：
...
### 10.7 `open_basedir restriction in effect`

`.user.ini` 限制了 PHP 只能访问 `public/` 目录。修复方法见 3.6 节。

### 10.8 SSL 证书申请失败

- **文件验证**：需要确保 Nginx 配置包含 `#error_page 404/404.html;` 标识
- **DNS 验证**：TXT 记录主机名填 `_acme-challenge`（不带域名后缀），值必须与宝塔显示的一致，且不要在确认前重新点申请（会生成新值）

### 10.9 Node.js 18 安装失败（CentOS 7）

CentOS 7 的 glibc 版本太低（2.17），需要升级到 CentOS 8+ 或 Ubuntu 20.04+。
