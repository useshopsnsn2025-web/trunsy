 ...
 *
 * 执行方式：
 * 1. 直接访问 /api/migrate?file=20260131000001_add_bri_translations
 * 2. 或手动执行以下 SQL
 * 3. 执行后需要清理缓存：删除 runtime/cache/ 目录下的缓存文件
 */
...
foreach ($translations as $locale => $items) {
    foreach ($items as $key => $value) {
        ...
        $sqlStatements[] = "INSERT INTO `ui_translations` (`locale`, `namespace`, `key`, `value`, `is_original`, `is_auto_translated`, `created_at`, `updated_at`)
VALUES ('{$locale}', '{$namespace}', '{$escapedKey}', '{$escapedValue}', 1, 0, '{$now}', '{$now}')
...
echo "请执行以下 SQL（共 " . count($sqlStatements) . " 条）:\n\n";
echo $sql;
echo "\n\n执行完成后，请删除 runtime/cache/ 目录下的缓存文件\n";
...
if (class_exists('think\facade\Db')) {
    try {
        ...
        echo "Migration executed successfully! 共插入/更新 " . count($sqlStatements) . " 条翻译记录\n";
        echo "请记得清理缓存：删除 runtime/cache/ 目录\n";
    } catch (\Exception $e) {
